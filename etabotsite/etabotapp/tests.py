...
class TMSModelTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.type = "JI"
        self.tms = TMS(owner=user, endpoint=self.endpoint, username=self.username, password=self.password, type=self.type)
...
class TMSViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        user = User.objects.create(username="kimchi", email="kimchi@etabot.ai", password="iloveelie")

        self.client = APIClient()
        self.client.force_authenticate(user=user)

        self.tms_data = {'owner': user.id, 'endpoint': 'https://etabot.atlassian.net', 'username': 'shanshan@etabot.ai', 'password': 'password', 'type': 'JI'}
        self.response = self.client.post('/tms/',
            self.tms_data,
            format="json")
    ...
    def test_authorization_is_enforced(self):
        ...
        res = new_client.get('/tms/', kwargs={'pk': 0}, format="json")
...
class ProjectModelTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.project = Project(owner=user, name=self.project_name, mode=self.project_mode, open_status=self.project_open_status, grace_period=self.project_grace_period, work_hours=self.project_work_hours, vacation_days=self.project_vacation_days)
...
class ProjectViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.project_data = {'owner': user.id, 'name': 'etabot', 'mode': 'scrum', 'open_status': 'ToDo', 'grace_period': '24h', 'work_hours': '1:(10,14)', 'vacation_days': '(2017-04-21, 2017-04-30)'}
        self.response = self.client.post('/projects/',
            #reverse('create'),
            self.project_data,
            format="json")
    ...
    def test_authorization_is_enforced(self):
        ...
        new_client = APIClient()
        res = new_client.get('/projects/', kwargs={'pk': 0}, format="json")
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_api_can_update_project(self):
        ...
        url = '{}{}{}'.format('/projects/update/', project.id, '/')
        res = self.client.put(url,
